...
if(isset($_GET['kode']))
{
	...
	$qygdihapus = que("SELECT * FROM tbl_petugas WHERE kode='".$_GET['kode']."';");
	$ygdihapus 	= fetch($qygdihapus);
	$hapus = mysql_query("DELETE FROM tbl_petugas WHERE kode='".$_GET['kode']."';");
	if($hapus)
	{
		echo "1";
		$log = $user['nama']." menghapus petugas ".$ygdihapus['nama'];
